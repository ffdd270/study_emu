...
using InstructionCallback = std::function<void( const char * instruction_name, BYTE opcode, WORD execute_pc_address )>;
...
public:
	...
	static std::shared_ptr<GameboyCPU> CreateWithMemoryInterface(std::shared_ptr<MemoryInterface> ptr_memory_interface );
	...
	[[nodiscard]] BYTE GetRegisterValueBy8BitIndex( BYTE index ) const {  return m8bitArguments[index].ref; }
...
private:
	...
	void addInstructionEvent(const char * name, BYTE opcode, WORD execute_pc_address);
