...
Clock::Clock(size_t clock_div) : mCycles(0 ), mClock(0), mClockDiv(clock_div )
{
...
{
	mCycles += clock;
	mClock += static_cast<size_t>( mCycles / mClockDiv );
	mCycles = mCycles % mClockDiv;
}
